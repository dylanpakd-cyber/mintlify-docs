---
title: "Set Up Proxies to Give Each Warmr iPhone Its Own IP"
sidebarTitle: "Proxies"
description: "Assign a dedicated ISP or SOCKS5 proxy to each iPhone when running two or more devices, then configure it with Stash or Shadowrocket."
---
...
<CardGroup cols={2}>
  ...
  <Card title="Scaling up → one IP per iPhone" icon="layers">
...
<CardGroup cols={2}>
  ...
  <Card title="4G mobile proxies" icon="signal">
    Traffic routes through a real mobile carrier network, which is the same path the phone would take on cellular data. The most faithful option, and the most expensive.
  </Card>
  <Card title="Residential proxies" icon="house">
    A cheaper alternative. IPs are assigned to real home connections rather than a datacenter. Choose providers with large, frequently rotated pools.
  </Card>
  <Card title="SIM card per iPhone" icon="signal">
    Ideal for hardware racks. Each iPhone uses its own SIM for mobile data, a unique carrier IP with no third-party proxy service at all.
  </Card>
</CardGroup>

**Why dedicated matters:** a dedicated IP is used by one iPhone only, so your devices do not share an origin and you do not inherit whatever the previous users of a shared pool were doing. Shared and rotating pools give you neither.

## What to avoid

<Warning>
  Steer clear of cheap shared pools, datacenter proxies, and free proxies. Their IPs are heavily reused and recycled, well known to posting history-and-safety systems, and one of the fastest ways to land in a reduced reach, no matter which brand sells them. Stick to dedicated ISP, residential, mobile, or SIM IPs.
</Warning>
