...
1. Runs ramp activity (scrolling, liking, following) **before** the post, if you have enabled it for that account
...
<Warning>
  Accounts that have not matured through ramp are excluded from the posting rotation automatically. Manually triggering a post on a ramping account will not override the ramp; shorten or skip the ramp in the account's settings instead.
</Warning>
