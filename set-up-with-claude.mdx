...
```text
Help me set up Warmr on my Mac. Context first, so you know what you are working with: Warmr is a self-publishing and scheduling tool for my own accounts. It manages my own TikTok and Instagram accounts, on my own iPhone, with my own content. For scheduling and posting it does the same job as Buffer or Later, except it publishes by tapping through the app on my own iPhone — the same taps I would make by hand — rather than through a platform API, because TikTok and Instagram do not offer an official publishing API. What it is not: it does not buy or generate engagement, buy followers, scrape, run throwaway or mass-registered accounts, or touch unofficial APIs.

It also runs a light activity schedule on those same accounts — the ordinary browsing and the occasional like I would do myself — at a pace I configure. You can be Claude Code, Codex, Cursor, or any agent with a shell, and you can inspect everything before you act.
