...
<AccordionGroup>
  ...
  <Accordion title="Will my accounts stay healthy over time?">
    No tool can guarantee outcomes — platform algorithms change, and enforcement is unpredictable. What Warmr controls is on your side: real iPhones, human-like taps and timing, and a warmup phase before every post. What you control is equally important: give each account a clean IP address and keep your posting cadence sane. Stacking many accounts on a single home IP is one of the fastest paths to flags, regardless of how well the on-device behavior is tuned.
  </Accordion>
  ...
  <Accordion title="Do I need proxies? Will my accounts get banned?">
    Bans come from network identity as much as on-device behavior. Running many accounts over a single home IP address is one of the fastest ways to get flagged — platforms see the shared origin even when the taps look human. Warmr handles the on-device behavior side; you handle the network side by giving each account its own mobile or residential IP, a 4G proxy, or a dedicated SIM. The exact proxy setup that works best for your configuration is something we walk through with you on a setup call.
  </Accordion>
