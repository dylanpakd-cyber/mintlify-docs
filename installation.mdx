---
title: "Install the Warmr app"
sidebarTitle: "Installation"
description: "Check the host and device requirements, install the Warmr app, activate your license key, and prepare your computer to control multiple iPhones."
---

Warmr runs on a desktop computer that drives your iPhones over USB. This page covers what you need, how to install the app, and how to handle the security prompt on first launch. The steps below show the Mac flow; Windows is supported too (see the requirements).

## Host requirements

<Tabs>
  <Tab title="Mac (recommended)">
    - **Apple Silicon** — M1, M2, M3, or M4. **Intel Macs are not supported.**
    - **macOS 14 or later.**
    - On-device screen recognition runs on the Neural Engine, so a Mac drives roughly twice as many iPhones as a comparable PC at the same budget.
  </Tab>
  <Tab title="Windows">
    - **Windows 10 22H2 or Windows 11**, 64-bit.
    - **iTunes installed from [apple.com](https://www.apple.com/itunes/)** — the standalone installer, not the Microsoft Store version. Warmr relies on the Apple device service only that installer provides.
    - Add the Warmr data folder to your antivirus / Defender exclusions, and don't run iTunes or "Apple Devices" alongside Warmr — they compete for the USB session.

    <Warning>
      Installing the runner with a free Apple ID needs Xcode, which is Mac-only. On Windows you must use the [App Store Connect API method](/setup/install-runner) (a paid Apple Developer account) to install the runner.
    </Warning>
  </Tab>
</Tabs>

### How many iPhones a host can drive
